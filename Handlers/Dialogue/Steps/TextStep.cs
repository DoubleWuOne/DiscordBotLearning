    ...
    public class TextStep : DialogueStepBase
    {
        ...
        public override async Task<bool>ProcessStep(DiscordClient client, DiscordChannel channel, DiscordUser user)
        {
            ...
            while (true)
            {
                ...
                if (_minLength.HasValue)
                {
                    if(messageResult.Result.Content.Length < _minLength)
                    {
                ...
                if (_maxLength.HasValue)
                {
                    if (messageResult.Result.Content.Length > _maxLength)
                    {
                        await TryAgain(channel, $"Your input is {messageResult.Result.Content.Length - _maxLength} characters too long").ConfigureAwait(false);
                        continue;
                    }
                }
